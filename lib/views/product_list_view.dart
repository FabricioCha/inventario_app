...
class _ProductListViewState extends State<ProductListView> {
  ...
  void _loadVisibleColumns() {
    ...
    if (savedColumns != null) {
      _visibleColumns = savedColumns.cast<String>().toList();
    }
  }
  ...
  void _updateAvailableColumns() {
    ...
    _allAvailableColumns = allKeys.toList()..sort();
  }
            ...
            return AlertDialog(
                      ...
                      child: ReorderableListView(
                        ...
                        onReorder: (int oldIndex, int newIndex) {
                          setDialogState(() {
                            ...
                            final String item = _allAvailableColumns.removeAt(
                              oldIndex,
                            );
                            _allAvailableColumns.insert(newIndex, item);

                            if (tempVisibleColumns.contains(item)) {
                              tempVisibleColumns.remove(item);
                              int visibleIndex = _allAvailableColumns
                                  .sublist(0, newIndex)
                                  .where((e) => tempVisibleColumns.contains(e))
                                  .length;
                              tempVisibleColumns.insert(visibleIndex, item);
                            }
                          });
                        },
                      ),
              ...
              actions: [
                ...
                TextButton(
                  ...
                  onPressed: () {
                    final settingsBox = Hive.box('settings');
                    settingsBox.put('visibleColumns', tempVisibleColumns);

                    if (_sortColumnIndex >= tempVisibleColumns.length) {
                      _sortColumnIndex = 0;
                    }

                    setState(() {
                      _visibleColumns = tempVisibleColumns;
                    });
